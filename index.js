...
client.on('message', message => {
	...
	switch (cmd) {
		...
		case 'pdir':
			...
			break;
		case 'logs':
			channel.send('Panel logs: `tail -n 100 /var/www/pterodactyl/storage/logs/laravel-$(date +%F).log | nc bin.ptdl.co 99`\nDaemon logs: `cd /srv/daemon/ && npm run diagnostics`');
			break;
		case 'install':
			channel.send('Install with:\n```bash <(curl -s https://raw.githubusercontent.com/vilhelmprytz/pterodactyl-installer/master/install.sh)```\nIf this fails, it is possible to do the same thing by running this:\n```wget https://raw.githubusercontent.com/vilhelmprytz/pterodactyl-installer/master/install.sh\nbash install.sh```');
			break;
		case 'check':
			channel.send('This command can be useful to check for errors on the panel:\n`/usr/bin/php /var/www/pterodactyl/artisan queue:work --queue=high,standard,low --sleep=3 --tries=3`');
			break;
		case 'firewall':
			channel.send('The installation scripts do not configure your firewall automatically. Here are the instructions for how to do so: <https://github.com/vilhelmprytz/pterodactyl-installer#firewall-setup>');
			break;
		case 'nstart':
		...
		case 'nrestart':
			...
			break;
		case 'nstatus':
			channel.send('Check status and logs of nginx with `systemctl status nginx`');
			break;
		case 'nstop':
			channel.send('Stop nginx with `systemctl stop nginx`');
			break;
		case 'wstart':
			channel.send('Start the daemon after configuration with `systemctl start wings`');
			break;
		case 'wrestart':
			channel.send('Restart the daemon with `systemctl restart wings`');
			break;
		case 'wstatus':
			channel.send('Check status and logs of the daemon with `systemctl status wings`');
			break;
		case 'wstop':
			channel.send('Stop the daemon with `systemctl stop wings`');
			break;
		case 'config':
			channel.send('The guide to configuring your daemon can be found here: <https://pterodactyl.io/daemon/0.6/installing.html#configure-daemon>');
			break;
		case 'storage':
			channel.send('The server files on the daemon are stored:\nnormally: in `/srv/daemon-data/`\non beta 1.0: in `/var/lib/pterodactyl/volumes/`');
			break;
		case 'muser':
			channel.send('Create a new panel user by running: ```cd /var/www/pterodactyl/\nphp artisan p:user:make```');
			break;
	}
});